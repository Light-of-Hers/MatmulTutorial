...
#define CO_VAR(__x) CAT(CUR_CORO, CAT(_, __x))
#define RESUME_POINT CO_VAR(resume_point)
#define CO_BEGIN         \
  int RESUME_POINT = -1; \
  auto CUR_CORO = [&]() -> bool { switch (RESUME_POINT) { case -1:
#define CO_END         \
  }                    \
  CO_RETURN;           \
  break_point:         \
...
__global__ void __launch_bounds__(NUM_THRDS, 1)
  ...
  auto get_next_mn_block = [&](uint32_t& m_blk_idx, uint32_t& n_blk_idx,
                               uint32_t& cur_mn_blk_idx) -> bool {
  ...
  if (warp_idx == 0) {  // TMA Load worker
    uint32_t m_blk_idx, n_blk_idx, cur_mn_blk_idx = blockIdx.x;
    while (get_next_mn_block(m_blk_idx, n_blk_idx, cur_mn_blk_idx)) {
    ...
    uint32_t sc_empty_idx = 0, sc_ready_idx = 0;
    constexpr uint32_t kNumStores = BLOCK_N / STORE_BLOCK_N;  // 256/64=4

#define CUR_CORO producer
#define m_blk_idx CO_VAR(m_blk_idx)
#define n_blk_idx CO_VAR(n_blk_idx)
#define cur_mn_blk_idx CO_VAR(cur_mn_blk_idx)
#define w CO_VAR(w)
#define s CO_VAR(s)
    uint32_t w, s, m_blk_idx, n_blk_idx, cur_mn_blk_idx = blockIdx.x;
    CO_BEGIN
    while (get_next_mn_block(m_blk_idx, n_blk_idx, cur_mn_blk_idx)) {
      ...
      UNROLL for (w = 0; w < NUM_UMMA_M_WAVES; ++w) {
        UNROLL for (s = 0; s < kNumStores; ++s) {
          ...
          UNROLL for (uint32_t i = 0;
            ...
            auto smem_ptr = reinterpret_cast<uint8_t*>(
                                smem_c[sc_ready_idx % NUM_SMEM_C_STAGES]) +
...
#undef m_blk_idx
#undef n_blk_idx
#undef cur_mn_blk_idx
#undef w
#undef s

#define CUR_CORO consumer
#define m_blk_idx CO_VAR(m_blk_idx)
#define n_blk_idx CO_VAR(n_blk_idx)
#define cur_mn_blk_idx CO_VAR(cur_mn_blk_idx)
#define w CO_VAR(w)
        ...
        UNROLL for (s = 0; s < kNumStores; ++s) {
          ...
          if (epi_thrd_idx == 0) {
            ...
            cute::SM90_TMA_STORE_2D::copy(
                &tensor_map_c, smem_c[sc_empty_idx % NUM_SMEM_C_STAGES], n_idx,
                m_idx);
          }
