...
void device_func(const cute::TmaDescriptor& A, const cute::TmaDescriptor& B, const cute::TmaDescriptor& C, int _lane_dim, int _warp_dim, int _warpgroup_dim, int _block_dim, int _cluster_dim, int _lane_idx, int _warp_idx, int _warpgroup_idx, int _block_idx, int _cluster_idx) {
  ...
  if (warpgroup_idx == 0 && warp_idx == 0)
    cute::TMEM::Allocator2Sm().allocate(
        512, reinterpret_cast<uint32_t*>(shared_storage));

  uint32_t C_TMEM[1] = {0};
  auto& A_SMEM_empty_chans =
      *reinterpret_cast<ClusterChannel<4>*>(shared_storage + 229376);
  auto& A_SMEM_ready_chans = *reinterpret_cast<ClusterChannel<4>*>(
      shared_storage + 229376 + sizeof(ClusterChannel<4>));
  auto& C_TMEM_empty_chans = *reinterpret_cast<ClusterChannel<1>*>(
      shared_storage + 229376 + sizeof(ClusterChannel<4>) +
      sizeof(ClusterChannel<4>));
  auto& C_TMEM_ready_chans = *reinterpret_cast<ClusterChannel<1>*>(
      shared_storage + 229376 + sizeof(ClusterChannel<4>) +
      sizeof(ClusterChannel<4>) + sizeof(ClusterChannel<1>));

  ARK_UNROLL for (int i = 0; i < 4; ++i) {
    if (warpgroup_idx == 0 && warp_idx == 0 && lane_idx == 0)
      A_SMEM_empty_chans[i].init(1);
  }
  ARK_UNROLL for (int i = 0; i < 4; ++i) {
    if (warpgroup_idx == 0 && warp_idx == 0 && lane_idx == 0)
      A_SMEM_ready_chans[i].init(2);
  }
  ARK_UNROLL for (int i = 0; i < 1; ++i) {
    if (warpgroup_idx == 0 && warp_idx == 0 && lane_idx == 0)
      C_TMEM_empty_chans[i].init(256);
  }
  ARK_UNROLL for (int i = 0; i < 1; ++i) {
    if (warpgroup_idx == 0 && warp_idx == 0 && lane_idx == 0)
      C_TMEM_ready_chans[i].init(1);
  }
  if (warpgroup_idx == 0 && warp_idx == 0 && lane_idx == 0) {
  ...
  if (warpgroup_idx == 0 && warp_idx == 0) {
    for (int cluster_idx = _cluster_idx; cluster_idx < 2048; cluster_idx += cluster_dim) {
      ...
      int sp_m_e16384s512_ = cluster_idx/(8*64)*8+cluster_idx%(8*64)%8, sp_n_e16384s256_ = cluster_idx%(8*64)/8;
      for (int k_e16384s64 = 0; k_e16384s64 < 256; ++k_e16384s64) {
        A_SMEM_empty_chans[(A_SMEM_state.index % A_SMEM_empty_chans.N_SLOTS)]
            .wait(A_SMEM_state.phase ^ 1);
        ...
        if (warpgroup_idx == 0 && warp_idx == 0 && lane_idx == 0)
          cute::SM100_TMA_2SM_LOAD_2D::copy(
              &A,
              reinterpret_cast<uint64_t*>(&A_SMEM_ready_chans[(
                  A_SMEM_state.index % A_SMEM_ready_chans.N_SLOTS)]),
              static_cast<uint32_t>(cute::TMA::CacheHintSm100::EVICT_NORMAL),
              ((bf16_t*)A_SMEM[A_SMEM_state.stage] + A_SMEM_off(0, 0)),
              k_e16384s64 * 64, sp_m_e16384s512_ * 512 + block_idx * 256); // FIXED: coords
        ...
        if (warpgroup_idx == 0 && warp_idx == 0 && lane_idx == 0)
          cute::SM100_TMA_2SM_LOAD_2D::copy(
              ...
              reinterpret_cast<uint64_t*>(&A_SMEM_ready_chans[(
                  A_SMEM_state.index % A_SMEM_ready_chans.N_SLOTS)]),
              static_cast<uint32_t>(cute::TMA::CacheHintSm100::EVICT_NORMAL),
              ((bf16_t*)B_SMEM[A_SMEM_state.stage] + B_SMEM_off(0, 0)),
              k_e16384s64 * 64, sp_n_e16384s256_ * 256 + block_idx * 128); // FIXED: coords
        if (warpgroup_idx == 0 && warp_idx == 0 && lane_idx == 0) {
          if (block_idx == 0)
            A_SMEM_ready_chans[(A_SMEM_state.index %
                                A_SMEM_ready_chans.N_SLOTS)]
                .arrive_and_expect_tx(65536 + 32768);
          else
            A_SMEM_ready_chans[(A_SMEM_state.index %
                                A_SMEM_ready_chans.N_SLOTS)]
                .arrive(0u);
          ...
          cute::SM100_MMA_F16BF16_2x1SM_SS<bf16_t, bf16_t, float, 256, 256,
              ...
              fma(make_8x128B_atom_smem_desc(
                  ...
                  cute::UMMA::make_runtime_instr_desc(
                      cute::UMMA::make_instr_desc<bf16_t, bf16_t, float, 256,
                                                  256, cute::UMMA::Major::K,
                                                  cute::UMMA::Major::K>()));
          C_TMEM_inited |= (1 << (C_TMEM_off1(0, 0, 0) / 256));
        }
    ...
    for (cluster_idx = _cluster_idx; cluster_idx < 2048; cluster_idx += cluster_dim) {
      ...
      sp_m_e16384s512_ = cluster_idx/(8*64)*8+cluster_idx%(8*64)%8, sp_n_e16384s256_ = cluster_idx%(8*64)/8;
      ...
      for (m_e256s128 = 0; m_e256s128 < 2; ++m_e256s128) for (n_e256s64 = 0; n_e256s64 < 4; ++n_e256s64) {
        ...
        for (n_e64s8 = 0; n_e64s8 < 8; ++n_e64s8) {
          ...
          cute::SM100_TMEM_LOAD_32dp32b8x::copy(
              ((uint32_t)C_TMEM[C_TMEM_state1.stage] + C_TMEM_off2(0)),
              ((u32_t*)C_REG)[C_REG_off(0)], ((u32_t*)C_REG)[C_REG_off(1)],
              ((u32_t*)C_REG)[C_REG_off(2)], ((u32_t*)C_REG)[C_REG_off(3)],
              ((u32_t*)C_REG)[C_REG_off(4)], ((u32_t*)C_REG)[C_REG_off(5)],
              ((u32_t*)C_REG)[C_REG_off(6)], ((u32_t*)C_REG)[C_REG_off(7)]);
          ...
          #define C_SMEM_off(i0, i1, i2) sp_m_e128s32_*2048+lane_idx*64+((lane_idx%8)^(n_e64s8))*8
          #define C_REG_BF16_off(i0) i0
          asm volatile("st.shared.v4.u32 [%0], {%1, %2, %3, %4};" ::"l"(
                           reinterpret_cast<uint8_t*>(
                               ((bf16_t*)C_SMEM[C_SMEM_state.stage] +
                                C_SMEM_off(0, 0, 0)))),
                       "r"(((u32_t*)C_REG_BF16)[C_REG_BF16_off(0)]),
                       "r"(((u32_t*)C_REG_BF16)[C_REG_BF16_off(1)]),
                       "r"(((u32_t*)C_REG_BF16)[C_REG_BF16_off(2)]),
                       "r"(((u32_t*)C_REG_BF16)[C_REG_BF16_off(3)]));
        }
        cutlass::arch::NamedBarrier(warpgroup_dim * warp_dim * lane_dim).sync();
        C_SMEM_state.next();
        CO_YIELD;
...
void matmul_sm100_bf16_2sm_256x256x64_kernel(const __grid_constant__ cute::TmaDescriptor A, const __grid_constant__ cute::TmaDescriptor B, const __grid_constant__ cute::TmaDescriptor C, int _lane_dim, int _warp_dim, int _warpgroup_dim, int _block_dim, int _cluster_dim) {
  ...
  // device_func(A, B, C, _lane_dim, _warp_dim, _warpgroup_dim, _block_dim, _cluster_dim, threadIdx.x, threadIdx.y, threadIdx.z, blockIdx.x % 2, blockIdx.x / 2);
}
extern "C" int matmul_sm100_bf16_2sm_256x256x64(bf16_t* A, bf16_t* B, bf16_t* C) {
  ...
  // matmul_sm100_bf16_2sm_256x256x64_kernel<<<dim3(2 * 76, 1, 1), dim3(32, 4, 2), smem_size>>>(A_desc, B_desc, C_desc, 32, 4, 2, 2, 76);
