...
void device_func(const cute::TmaDescriptor& A, const cute::TmaDescriptor& B, const cute::TmaDescriptor& C, int _lane_dim, int _warp_dim, int _warpgroup_dim, int _block_dim, int _cluster_dim, int _lane_idx, int _warp_idx, int _warpgroup_idx, int _block_idx, int _cluster_idx) {
  ...
  uint32_t C_TMEM[1] = {0};
  auto& A_SMEM_empty_chans = *reinterpret_cast<ClusterChannel<4>*>(shared_storage + 229376);
  auto& A_SMEM_ready_chans = *reinterpret_cast<ClusterChannel<4>*>(shared_storage + 229376 + sizeof(ClusterChannel<4>));
  auto& B_SMEM_empty_chans = *reinterpret_cast<ClusterChannel<4>*>(shared_storage + 229376 + sizeof(ClusterChannel<4>) + sizeof(ClusterChannel<4>));
  auto& B_SMEM_ready_chans = *reinterpret_cast<ClusterChannel<4>*>(shared_storage + 229376 + sizeof(ClusterChannel<4>) + sizeof(ClusterChannel<4>) + sizeof(ClusterChannel<4>));
  auto& C_TMEM_empty_chans = *reinterpret_cast<ClusterChannel<1>*>(shared_storage + 229376 + sizeof(ClusterChannel<4>) + sizeof(ClusterChannel<4>) + sizeof(ClusterChannel<4>) + sizeof(ClusterChannel<4>));
  auto& C_TMEM_ready_chans = *reinterpret_cast<ClusterChannel<1>*>(shared_storage + 229376 + sizeof(ClusterChannel<4>) + sizeof(ClusterChannel<4>) + sizeof(ClusterChannel<4>) + sizeof(ClusterChannel<4>) + sizeof(ClusterChannel<1>));
  ...
  ARK_UNROLL for (int i = 0; i < 4; ++i) {
    if (warpgroup_idx == 0 && warp_idx == 0 && lane_idx == 0) B_SMEM_empty_chans[i].init(1);
  }
  ARK_UNROLL for (int i = 0; i < 4; ++i) {
    if (warpgroup_idx == 0 && warp_idx == 0 && lane_idx == 0) B_SMEM_ready_chans[i].init(2);
  }
  ...
  PipelineState<4> B_SMEM_state;
  PipelineState<1> C_TMEM_state;
  PipelineState<4> A_SMEM_state1;
  PipelineState<4> B_SMEM_state1;
  ...
  if (warpgroup_idx == 0 && warp_idx == 0) {
    for (int cluster_idx = _cluster_idx; cluster_idx < 2048; cluster_idx += cluster_dim) {
      int sp_m_e16384s512_ = cluster_idx/512*8+cluster_idx%512%min(8, 32-cluster_idx/512*8), sp_n_e16384s256_ = cluster_idx%512/min(8, 32-cluster_idx/512*8);
      for (int k_e16384s64 = 0; k_e16384s64 < 256; ++k_e16384s64) {
        ...
        #define A_SMEM_off(i0, i1, i2) (i0*64+i2+((i0)%8^(i1))*8)
        #define A_off(i0, i1, i2, i3) (sp_m_e16384s512_*8388608+block_idx/1%2*4194304+k_e16384s64*64+i1*16384+i2*8+i3)
        if (warpgroup_idx == 0 && warp_idx == 0 && lane_idx == 0) cute::SM100_TMA_2SM_LOAD_2D::copy(&A, reinterpret_cast<uint64_t*>(&A_SMEM_ready_chans[(A_SMEM_state.index % A_SMEM_ready_chans.N_SLOTS)]), static_cast<uint32_t>(cute::TMA::CacheHintSm100::EVICT_NORMAL), ((bf16_t*)A_SMEM[A_SMEM_state.stage]+A_SMEM_off(0, 0, 0)), k_e16384s64*64, sp_m_e16384s512_*512+block_idx/1%2*256);
        if (warpgroup_idx == 0 && warp_idx == 0 && lane_idx == 0) { if (block_idx == 0) A_SMEM_ready_chans[(A_SMEM_state.index % A_SMEM_ready_chans.N_SLOTS)].arrive_and_expect_tx(65536); else A_SMEM_ready_chans[(A_SMEM_state.index % A_SMEM_ready_chans.N_SLOTS)].arrive(0u); }
        A_SMEM_state.next();
        B_SMEM_empty_chans[(B_SMEM_state.index % B_SMEM_empty_chans.N_SLOTS)].wait(B_SMEM_state.phase ^ 1);
        #define B_SMEM_off(i0, i1, i2) (i0*64+i2+((i0)%8^(i1))*8)
        #define B_off(i0, i1, i2, i3) (sp_n_e16384s256_*4194304+block_idx/1%2*2097152+k_e16384s64*64+i1*16384+i2*8+i3)
        if (warpgroup_idx == 0 && warp_idx == 0 && lane_idx == 0) cute::SM100_TMA_2SM_LOAD_2D::copy(&B, reinterpret_cast<uint64_t*>(&B_SMEM_ready_chans[(B_SMEM_state.index % B_SMEM_ready_chans.N_SLOTS)]), static_cast<uint32_t>(cute::TMA::CacheHintSm100::EVICT_NORMAL), ((bf16_t*)B_SMEM[B_SMEM_state.stage]+B_SMEM_off(0, 0, 0)), k_e16384s64*64, sp_n_e16384s256_*256+block_idx/1%2*128);
        if (warpgroup_idx == 0 && warp_idx == 0 && lane_idx == 0) { if (block_idx == 0) B_SMEM_ready_chans[(B_SMEM_state.index % B_SMEM_ready_chans.N_SLOTS)].arrive_and_expect_tx(32768); else B_SMEM_ready_chans[(B_SMEM_state.index % B_SMEM_ready_chans.N_SLOTS)].arrive(0u); }
        B_SMEM_state.next();
      }
    }
  }
    ...
    for (int cluster_idx = _cluster_idx; cluster_idx < 2048; cluster_idx += cluster_dim) {
      ...
      for (int k_e16384s64 = 0; k_e16384s64 < 256; ++k_e16384s64) {
        ...
        B_SMEM_ready_chans[(B_SMEM_state1.index % B_SMEM_ready_chans.N_SLOTS)].wait(B_SMEM_state1.phase);
        for (int k_e64s16 = 0; k_e64s16 < 4; ++k_e64s16) for (int m_e256s128 = 0; m_e256s128 < 2; ++m_e256s128) {
          ...
          #define A_SMEM_off1(i0, i1, i2) (m_e256s128*8192+i0*512+i1*64+i2%8+((m_e256s128*128+i0*8+i1)%8^(k_e64s16*2+i2/8))*8)
          #define B_SMEM_off1(i0, i1, i2) (i0*512+i1*64+i2%8+((i0*8+i1)%8^(k_e64s16*2+i2/8))*8)
          cute::SM100_MMA_F16BF16_2x1SM_SS<bf16_t, bf16_t, float, 256, 256, cute::UMMA::Major::K, cute::UMMA::Major::K>::fma(make_8x128B_atom_smem_desc(static_cast<uint16_t>(cute::cast_smem_ptr_to_uint(((bf16_t*)A_SMEM[A_SMEM_state1.stage]+A_SMEM_off1(0, 0, 0))) >> 4)), make_8x128B_atom_smem_desc(static_cast<uint16_t>(cute::cast_smem_ptr_to_uint(((bf16_t*)B_SMEM[B_SMEM_state1.stage]+B_SMEM_off1(0, 0, 0))) >> 4)), ((uint32_t)C_TMEM[C_TMEM_state.stage]+C_TMEM_off1(0, 0, 0)), (C_TMEM_inited & (1 << (C_TMEM_off1(0, 0, 0) / 256))) != 0, cute::UMMA::make_runtime_instr_desc(cute::UMMA::make_instr_desc<bf16_t, bf16_t, float, 256, 256, cute::UMMA::Major::K, cute::UMMA::Major::K>())); C_TMEM_inited |= (1 << (C_TMEM_off1(0, 0, 0) / 256));
        }
        ...
        A_SMEM_state1.next();
        cutlass::arch::umma_arrive_multicast_2x1SM(reinterpret_cast<uint64_t*>(&B_SMEM_empty_chans[(B_SMEM_state1.index % B_SMEM_empty_chans.N_SLOTS)]), 3);
        B_SMEM_state1.next();
      }
    ...
    int n_e256s64, sp_m_e16384s512_, n_e8s1, n_e64s8, cluster_idx, m_e256s128, sp_n_e16384s256_;
    CO_BEGIN
    for (cluster_idx = _cluster_idx; cluster_idx < 2048; cluster_idx += cluster_dim) {
      sp_m_e16384s512_ = cluster_idx/512*8+cluster_idx%512%min(8, 32-cluster_idx/512*8), sp_n_e16384s256_ = cluster_idx%512/min(8, 32-cluster_idx/512*8);
      ...
      for (m_e256s128 = 0; m_e256s128 < 2; ++m_e256s128) for (n_e256s64 = 0; n_e256s64 < 4; ++n_e256s64) {
        ...
        for (n_e64s8 = 0; n_e64s8 < 8; ++n_e64s8) {
          ...
          #define C_SMEM_off(i0, i1, i2) (sp_m_e128s32_*2048+lane_idx/1%32*64+i1*2+i2+((sp_m_e128s32_*32+lane_idx/1%32)%8^(n_e64s8))*8)
      ...
      C_TMEM_empty_chans[(C_TMEM_state1.index % C_TMEM_empty_chans.N_SLOTS)].arrive(0u); C_TMEM_empty_chans[(C_TMEM_state1.index % C_TMEM_empty_chans.N_SLOTS)].arrive(1u);
      C_TMEM_state1.next();
    }
    CO_END
    ...
    #define m_e256s128 CO_VAR(m_e256s128)
    #define sp_n_e16384s256_ CO_VAR(sp_n_e16384s256_)
    int n_e256s64, sp_m_e16384s512_, cluster_idx, m_e256s128, sp_n_e16384s256_;
    CO_BEGIN
    for (cluster_idx = _cluster_idx; cluster_idx < 2048; cluster_idx += cluster_dim) {
      sp_m_e16384s512_ = cluster_idx/512*8+cluster_idx%512%min(8, 32-cluster_idx/512*8), sp_n_e16384s256_ = cluster_idx%512/min(8, 32-cluster_idx/512*8);
      for (m_e256s128 = 0; m_e256s128 < 2; ++m_e256s128) for (n_e256s64 = 0; n_e256s64 < 4; ++n_e256s64) {
        while (C_SMEM_state.index <= C_SMEM_state1.index) CO_YIELD;
        #define C_off(i0, i1, i2) (sp_m_e16384s512_*8388608+sp_n_e16384s256_*256+sp_m_e512s256_*4194304+m_e256s128*2097152+n_e256s64*64+i0*16384+i1*8+i2)
        #define C_SMEM_off1(i0, i1, i2) (i0*64+i2+((i0)%8^(i1))*8)
        if (warpgroup_idx == 0 && warp_idx == 0 && lane_idx == 0) cute::SM90_TMA_STORE_2D::copy(&C, ((bf16_t*)C_SMEM[C_SMEM_state1.stage]+C_SMEM_off1(0, 0, 0)), sp_n_e16384s256_*256+n_e256s64*64, sp_m_e16384s512_*512+sp_m_e512s256_*256+m_e256s128*128);
...
void matmul_sm100_bf16_2sm_256x256x64_kernel(const __grid_constant__ cute::TmaDescriptor A, const __grid_constant__ cute::TmaDescriptor B, const __grid_constant__ cute::TmaDescriptor C, int _lane_dim, int _warp_dim, int _warpgroup_dim, int _block_dim, int _cluster_dim) {
...
extern "C" int matmul_sm100_bf16_2sm_256x256x64(bf16_t* A, bf16_t* B, bf16_t* C) {
  constexpr uint32_t smem_size = 229376 + sizeof(ClusterChannel<4>) + sizeof(ClusterChannel<4>) + sizeof(ClusterChannel<4>) + sizeof(ClusterChannel<4>) + sizeof(ClusterChannel<1>) + sizeof(ClusterChannel<1>);
  static bool init_flag = false;
  if (!init_flag) {
    CUDA_CHECK(cudaFuncSetAttribute(matmul_sm100_bf16_2sm_256x256x64_kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size), return -1);
    init_flag = true;
  }
  ...
  matmul_sm100_bf16_2sm_256x256x64_kernel<<<dim3(2, 76, 1), dim3(32, 4, 2), smem_size>>>(A_desc, B_desc, C_desc, 32, 4, 2, 2, 76);
  return 0;
}
